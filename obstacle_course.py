...
CRASH_SOUND = pygame.mixer.Sound(os.path.join('Assets_game', 'crash.wav'))
...
def draw_win(bg, man, obstacles, man_health, score, highscore):
    ...
    ROCK = pygame.transform.scale(pygame.image.load(os.path.join('Assets_Game','shark.png')),(ROCK_WIDTH, ROCK_HEIGHT))
...
def main():
    ...
    highscore = max(recs)
    while run:
        bg = pygame.transform.scale((pygame.image.load(os.path.join('Assets_Game','ocean.jpg'))),(WIDTH,HEIGHT))
